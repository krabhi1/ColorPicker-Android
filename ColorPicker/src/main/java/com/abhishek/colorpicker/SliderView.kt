...
open class SliderView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    protected var ratioChangeListener:((t:Float)->Unit)?=null
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        super.onSizeChanged(w, h, oldw, oldh)
    }
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        super.onMeasure(widthMeasureSpec, heightMeasureSpec)
        val size=Utils.setWrapSize(100.toPx,20.toPx,widthMeasureSpec,heightMeasureSpec)
        setMeasuredDimension(size.width,size.height)
    }

    protected  fun getRadius():Int{
        val maxRadius=30.toPx
        var radius=  measuredHeight*0.5f
        if(radius>maxRadius){
            radius= maxRadius.toFloat()
        }
        return radius.toInt()
    }
    protected fun maxTrackSize()=measuredWidth-getRadius()*2f
    protected fun updateRatio(e:MotionEvent){
        val rad=getRadius()
    ...
    protected fun drawWheel(canvas:Canvas){
        //draw wheel
        val halfHeight=measuredHeight*0.5f
        val x= getRadius()+maxTrackSize()*currentRatio
        val y=halfHeight
        canvas.drawCircle(x,y,getRadius().toFloat(),wheelPaint)
    }
    //public
